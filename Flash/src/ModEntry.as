package  {
	...
	public class ModEntry extends MovieClip {
		...
		private var descriptionWidth:Number;
		private var descriptionHeight:Number;
		...
		public function ModEntry() {
			...
			//TODO: Get these values from API or from constructor itself
			SetImages(new Array("ModDotaClient2/test.png","ModDotaClient2/test2.png"));
		...
		public function SetImages(imageArray:Array) {
			//todo: validation
			this.imageArray = imageArray;
			trace("###HELPME Set ImageArray to:");
			for (var image in imageArray) {
				trace("###HELPME    "+image+": "+imageArray[image]);
			}
			trace("###HELPME End ImageArray");
			Globals.instance.LoadImageWithCallback(imageArray[0], imageLocation, true, imageLoaded);
		}
		public function adButtonClicked(e:ButtonEvent) {
			buttonID = e.target["AdButtonID"];
			adButton1.selected = (buttonID == adButton1["AdButtonID"]);
			adButton2.selected = (buttonID == adButton2["AdButtonID"]);
			trace("###HELPME Loading "+imageArray[buttonID-1]);
			Globals.instance.LoadImageWithCallback(imageArray[buttonID-1], imageLocation, true, imageLoaded);
		}
